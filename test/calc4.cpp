/*
 * @Author: Elendeer
 * @Date: 2020-05-27 20:28:18
 * @LastEditors: Elendeer
 * @LastEditTime: 2020-05-29 11:43:49
...
enum TokenType { // define enum type to record the type of a token.
    NONE, INTEGER, MUL, DIV, eEOF
};
const string getStringFromEnum[] = {"NONE", "INTEGER", "MUL", "DIV", "EOF"};
...
/**
 * @description: Return an INTEGER token value.
 * factor : INTEGER
 * @param {}
 * @return: int
 */
...
int Interpreter::expr() {
    ...
    while (m_current_token.m_type == MUL || m_current_token.m_type == DIV) {
        ...
        else if (m_current_token.m_type == DIV) {
            eat(DIV);
            result /= factor();
        }
    }
